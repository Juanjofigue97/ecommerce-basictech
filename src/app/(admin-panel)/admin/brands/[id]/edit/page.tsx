...
export default function EditBrandPage() {
  ...
  useEffect(() => {
    fetch(`/api/brands/${id}`)
      .then((r) => r.json())
      .then((data) => {
        reset({ name: data.name, slug: data.slug, logo: data.logo ?? "" })
        setLoading(false)
      })
  }, [id, reset])
  ...
  async function onSubmit(data: FormData) {
    setSaving(true)
    try {
      await updateBrand(id, { name: data.name, slug: data.slug, logo: data.logo || undefined })
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div>
          ...
          <p className="text-muted-foreground">Actualiza los datos de la marca</p>
        </div>
      </div>
